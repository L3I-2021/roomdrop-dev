...
class HostFS(Fuse):
    ...
    def unlink(self, path):
        guest_folder = client.credentials['guest']['fullname']

        if path.startswith('/' + guest_folder):
            client.delete(path)
        else:
            os.unlink("." + path)
    ...
    class MeetingFile(object):
        def __init__(self, path, flags, *mode):
            ...
            self.empty_write_finished = False
            ...
            # If file is in the public folder then download it
...
def main():
    usage = """
        Roomdrop Guest POV meeting filesystem
    """ + Fuse.fusage
